...
;; Root owner managing the entire registry
(define-data-var root-registry-owner principal tx-sender)
...
(define-private (is-contract-owner)
  (is-eq tx-sender (var-get root-registry-owner))
)
...
;; Transfer registry root ownership with strict authorization
(define-public (transfer-registry-ownership (new-root-owner principal))
  (begin
    (asserts! (is-contract-owner) ERR-NOT-AUTHORIZED)
    (ok (var-set root-registry-owner new-root-owner))
  )
)

;; Register a new game
(define-public (add-platform-entry 
  (game-id (string-ascii 50)) 
  (name (string-ascii 100)) 
  (website-url (optional (string-ascii 255))) 
  (description (string-utf8 500)))
